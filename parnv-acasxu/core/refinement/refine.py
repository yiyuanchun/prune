...
from core.utils.layer_scope import (
    get_last_hidden_layer_indices,
    is_node_name_in_layer_indices,
)
...
def global_refine(network: Network, processed_net, ori_var2val, actions, **kws):
    ...
    refine_part = network.get_global_refine_part(network, network.orig_name2node_map, example, processed_net,
                                                 ori_var2val, actions)
    if not refine_part:
        print("No global refinement candidate is available; returning the network unchanged.")
        network.global_refine_no_candidate = True
        return network
    network.global_refine_no_candidate = False
    print(refine_part)
    ...
    if union_node_name not in network.name2node_map:
        print("The selected global refinement part is not present in the current network; returning unchanged.")
        network.global_refine_no_candidate = True
        return network
...
def get_refinement_sequence(network, sequence_len=1, **kws) -> List[float]:
    ...
    refine_layer_indices = get_last_hidden_layer_indices(network, count=2)
    part2loss_map = {
        part: loss
        for part, loss in part2loss_map.items()
        if is_node_name_in_layer_indices(part, refine_layer_indices)
    }
