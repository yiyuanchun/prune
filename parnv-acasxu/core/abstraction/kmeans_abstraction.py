...
def kmeans_abstraction_based_on_contribution(network: Network, test_property: dict, do_preprocess: bool = True,
    ...
    while not has_violation(network, test_property, random_inputs):
        ...
        for inode in network.layers[index].nodes:
            if inode != candicate_node:
                if inode.name.split("_")[3] == candicate_node.name.split("_")[3].split('+')[0] and not inode.deleted:
                    combine_influ_list.append(
                        (inode.name, combin_influnce(inode, candicate_node, network, index, nodes2edge_between_map)))
        ...
        if not may_combine_item or delete_inf <= may_combine_item[1]:
            ...
            finish_abstraction(network=network,
                               ...
                               verbose=VERBOSE)
            nodename2contribution_map[union_name] = nodename2contribution_map[candicate_node.name] + \
                                                    nodename2contribution_map[may_combine_item[0]]
            del nodename2contribution_map[candicate_node.name]
