...
def classify_crown_output_bounds(
    ...
    if violation_operator == "ge":
        for output_index in output_indices:
            if output_bounds.lower_bounds[output_index] >= 0.0:
                return (
                    "SAT",
                    "CROWN proved adversarial output {} is always >= 0.".format(output_index),
                )

        if all(output_bounds.upper_bounds[output_index] < 0.0 for output_index in output_indices):
            return (
                "UNSAT",
                "CROWN proved every adversarial output upper bound is < 0.",
            )
    elif violation_operator == "le":
        for output_index in output_indices:
            if output_bounds.upper_bounds[output_index] <= 0.0:
                return (
                    "SAT",
                    "CROWN proved adversarial output {} is always <= 0.".format(output_index),
                )

        if all(output_bounds.lower_bounds[output_index] > 0.0 for output_index in output_indices):
            return (
                "UNSAT",
                "CROWN proved every adversarial output lower bound is > 0.",
            )
    else:
        raise ValueError(
