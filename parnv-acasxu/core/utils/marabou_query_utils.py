...
def _build_ge_zero_equation(variable: int, coefficient: float = 1.0):
    equation = None
...
def handle_adversarial(net, test_property) -> Tuple:
    ...
    # print(net.layers[-2])
    test_property["output"] = [
        (i, {"Lower": 0.0}) for i in range(len(new_last_hidden_nodes))
    ]
    test_property["_adversarial_target_label"] = minimum_ind
    test_property["_adversarial_non_target_labels"] = non_target_indices
    # print(test_property["output"])
...
def get_query(
    ...
    if (
        ...
        for var_index, _ in test_property["output"]:
            output_var = out_layer_var_index + var_index
            if violation_operator == "le":
                disjunction_clauses.append([_build_ge_zero_equation(output_var, coefficient=-1.0)])
                if verbose:
                    print("add adversarial disjunct: output var {} <= 0".format(output_var))
