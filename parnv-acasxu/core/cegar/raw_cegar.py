...
def _write_stage_logs(
        state: RawCegarState,
        output_dir: Path,
        cegar_log: dict[str, Any] | None = None,
        write_detailed_results: bool = True,
) -> None:
    if not write_detailed_results:
        return
...
def verify_network_with_marabou(
    ...
    try:
        values, stats, status = get_query(
            ...
            verbose=False,
            marabou_timeout_seconds=1200 if timeout is None else int(timeout),
        ...
        return {
            "status": status,
            "counterexample": {},
            "marabou_runtime": time.perf_counter() - started,
            "raw_solver_output": "{}: {}".format(type(exc).__name__, exc),
        }
    normalized_status = _normalize_marabou_status(status)
    return {
...
def direct_verify_with_marabou(
    ...
    if "type" not in raw_property:
        if property_id is not None and property_id in raw_property:
            ...
            raise ValueError("A single ACASXu property with a 'type' field is required.")

    network = network_from_nnet_file(str(original_nnet_file))
    from core.utils.marabou_query_utils import reduce_property_to_basic_form

    network, test_property = reduce_property_to_basic_form(
        network=network,
    ...
    crown_hidden_bounds, crown_output_bounds = compute_crown_bounds(network)
    crown_time = time.perf_counter() - crown_started
    crown_status, crown_reason = classify_crown_output_bounds(
        test_property=test_property,
        output_bounds=crown_output_bounds,
    )

    result: dict[str, Any] = {
        "crown_precheck_status": crown_status,
        "crown_precheck_reason": crown_reason,
        "crown_time": crown_time,
        "crown_output_lower_bounds": crown_output_bounds.lower_bounds,
        "crown_output_upper_bounds": crown_output_bounds.upper_bounds,
        "adversarial_output_order": test_property.get("_adversarial_output_order", ""),
        "adversarial_query_mode": test_property.get("_adversarial_query_mode", ""),
        "adversarial_margin": test_property.get("_adversarial_margin", ""),
        "adversarial_violation_operator": test_property.get("_adversarial_violation_operator", ""),
        "num_of_refine_steps": 0,
        "abstraction_time": 0.0,
        "total_refinement_steps": 0,
    }

    if crown_status in {"SAT", "UNSAT"}:
        result.update({
            "result": "UNSAFE" if crown_status == "SAT" else "VERIFIED",
            "query_result": crown_status,
            "reason": crown_reason,
            "dead_relu_pruned": 0,
        ...
        if write_detailed_results:
            write_nnet(network, output_path / "crown_prechecked_network.nnet")
            _write_json(output_path / "direct_result.json", result)
            _write_json(output_path / "counterexample.json", {})
        return result
    ...
    if write_detailed_results:
        write_nnet(network, output_path / "crown_pruned_network.nnet")
    verify_result = verify_network_with_marabou(
        network=network,
        test_property=test_property,
        timeout=marabou_timeout,
        save_query_path=output_path / "crown_pruned.query" if write_detailed_results else None,
    )
    status = verify_result["status"]
    query_result = status.upper()
    result.update({
        "result": (
            "VERIFIED" if status == "unsat"
            else "UNSAFE" if status == "sat"
            else "TIMEOUT" if status == "timeout"
            else "ERROR" if status == "error"
            else "UNKNOWN"
        ),
        "query_result": query_result,
        "reason": "Marabou returned {} on the CROWN-pruned network".format(status),
        "dead_relu_pruned": pruning_report.get("total_pruned", 0),
        "dead_relu_pruning_report": pruning_report,
        "pruned_query_time": verify_result["marabou_runtime"],
        "elapsed_seconds": time.perf_counter() - started,
    })
    counterexample = (
        {"input_assignment": verify_result["counterexample"]}
        if status == "sat"
        else {}
    )
    if write_detailed_results:
        _write_json(output_path / "direct_result.json", result)
        _write_json(output_path / "counterexample.json", counterexample)
    return result
...
def verify_with_marabou(
    ...
    apply_input_bounds_from_property(network, test_property)
    return verify_network_with_marabou(network, test_property, timeout=timeout)


def _normalize_marabou_status(status: Any) -> str:
...
def _precheck_final_result(
    ...
    if status == "UNSAT":
        ...
        if write_detailed_results:
            final_result["final_network"] = "refined_network_final.nnet"
    ...
    if genuine:
        ...
        if write_detailed_results:
            final_result["counterexample_file"] = "counterexample.json"
...
def cegar_verify_with_marabou(
    ...
    pruned_reference_network = build_network_from_state(state)
    preprocess_report = preprocess_last_two_hidden_layers_inc_dec(state)
    cegar_log["inc_dec_preprocess_report"] = preprocess_report
    if write_detailed_results:
        write_nnet(state, output_path / "preprocessed_equivalent.nnet")
    equivalence_report = _random_equivalence_test(
        reference_network=pruned_reference_network,
        state=state,
        samples=20,
        tolerance=1e-6,
    )
    cegar_log["inc_dec_equivalence_test"] = equivalence_report
    if not equivalence_report["passed"]:
        final_result = {
            "result": "UNKNOWN",
            "query_result": "UNKNOWN",
            "reason": "inc/dec preprocessing failed random equivalence test",
    ...
    while True:
        ...
        verify_result = verify_network_with_marabou(
            ...
            test_property=test_property,
            timeout=marabou_timeout,
            save_query_path=query_path if write_detailed_results else None,
        )
        ...
        if verify_result["status"] == "unsat":
            final_result = {
                "result": "VERIFIED",
                "query_result": "UNSAT",
                "reason": "Marabou returned unsat on current abstract/refined network",
        ...
        final_result = {
            ...
            "reason": "Marabou returned {}".format(verify_result["status"]),
            "iterations": iteration,
            "total_refinement_steps": total_refinement_steps,
            "dead_relu_pruned": pruning_report.get("total_pruned", 0),
            "elapsed_seconds": time.perf_counter() - started,
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run parnv-acasxu local Merge CEGAR on one .nnet/property pair.")
    ...
    parser.add_argument(
        "--no-detailed-results",
        action="store_true",
        help="Do not write JSON logs, counterexamples, queries, or intermediate .nnet artifacts.",
    )
...
def main() -> None:
    ...
    result = cegar_verify_with_marabou(
        ...
        write_detailed_results=not args.no_detailed_results,
    )
