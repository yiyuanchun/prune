...
class Network:
    ...
    def __init__(self, layers: List, weights: List = None,
                 biases: List = None, acasxu_net=None):
        self.layers = layers
    ...
    def get_global_refine_part(self,
        ...
        if refineables:
            ...
            for deleted_name in refine_list:
                ...
                for out_edge in deleted_node.out_edges:
                    ...
                    num_of_outedges += 1
                layer_index = int(deleted_name.split("_")[1])
                layer_max = max_of_each_layer[layer_index] if layer_index < len(max_of_each_layer) else 0
                if num_of_outedges == 0:
                    node2loss[deleted_name] = 0
                elif layer_max != 0:
                    node2loss[deleted_name] = abs(deleted_node_inf) / (layer_max * num_of_outedges)
                elif deleted_node_inf != 0:
        ...
        # self.generate_name2node_map()
        if not node2loss:
            print("No global refinement candidate was found.")
            return []
