...
class ARNode:
    ...
    def __init__(self, name: AnyStr, ar_type: AnyStr, in_edges: List, out_edges: List,
                 ...
                 lower_bound: float = 0.0, preactivation_upper_bound: float = None,
                 preactivation_lower_bound: float = None):
        self.name = name
        ...
        self.preactivation_upper_bound = (
            upper_bound if preactivation_upper_bound is None else preactivation_upper_bound
        )
        self.preactivation_lower_bound = (
            lower_bound if preactivation_lower_bound is None else preactivation_lower_bound
        )
    ...
    def split_pos_neg(self, name2node_map: Dict) -> List:
        ...
        node_pos = ARNode(name=self.name + "_pos",
                          ...
                          preactivation_upper_bound=self.preactivation_upper_bound,
                          preactivation_lower_bound=self.preactivation_lower_bound
                          )
    ...
    def cal_contri(self) -> float:
        upper_bound = getattr(self, "preactivation_upper_bound", self.upper_bound)
        lower_bound = getattr(self, "preactivation_lower_bound", self.lower_bound)
