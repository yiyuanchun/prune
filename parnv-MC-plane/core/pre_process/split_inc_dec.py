...
def preprocess_split_inc_dec(network: Network, layer_indices=None) -> None:
    """
    ...
    if layer_indices is None:
        ...
        target_hidden_layers = sorted({
            int(index)
            for index in layer_indices
            if 0 < int(index) < len(network.layers) - 1
        })
        if not target_hidden_layers:
            return
        target_split_layers = set(target_hidden_layers)
        target_split_layers.add(len(network.layers) - 1)
        for i in sorted(target_split_layers, reverse=True):
            network.layers[i].split_inc_dec(network.name2node_map)
        network.generate_name2node_map()
        boundary_layer_index = target_hidden_layers[0] - 1
        if boundary_layer_index >= 0:
            adjust_layer_after_split_inc_dec(network, layer_index=boundary_layer_index)
    if VERBOSE:
