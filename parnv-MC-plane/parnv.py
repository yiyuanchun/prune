...
def one_experiment(
        ...
        run_args=None, planet_timeout_seconds=None, planet_bin=None,
        marabou_timeout_seconds=None
):
    """
    ...
    from core.utils.planet_query_utils import get_query
    ...
    if planet_timeout_seconds is None:
        planet_timeout_seconds = marabou_timeout_seconds
    effective_planet_timeout_seconds = (
        1200
        if planet_timeout_seconds is None
        else int(planet_timeout_seconds)
    )
    print("verification flow: CROWN dead-ReLU pruning -> Planet")
    print("planet_timeout_seconds={}".format(effective_planet_timeout_seconds))
    marabou_timeout_seconds = effective_planet_timeout_seconds
    ...
    base_res = [
        ...
        ("property_id", property_id),
        ("verification_flow", "crown_dead_relu_pruning_then_planet"),
        ...
        ("planet_timeout_seconds", effective_planet_timeout_seconds),
    ...
    print("query using Planet on CROWN-pruned network")
    ...
    vars1, stats1, query_result = get_query(
        ...
        planet_timeout_seconds=effective_planet_timeout_seconds,
        save_query_path=os.path.join(results_directory, "crown_pruned.rlv"),
    ...
    # if mechanism is vanilla planet
    if mechanism in {"marabou", "planet"}:
        print("query using vanilla Planet")
        ...
        t1 = time.time()
        # time to check property on net with planet
        marabou_time = t1 - t0
        if verbose:
    ...
    # otherwise mechanism is planet_with_ar
    orig_net = copy.deepcopy(net)
    print("query using Planet with AR")
    t2 = time.time()
    ...
    # time to check property on net with planet using CEGAR
    total_ar_time = t3 - t2
    if verbose:
...
def parse_args():
    ...
    parser.add_argument("--planet-timeout-seconds",
                        "--marabou-timeout-seconds",
                        dest="planet_timeout_seconds",
                        type=int,
                        default=1200,
                        help="Planet solver timeout in seconds.")
    parser.add_argument("--planet-bin",
                        dest="planet_bin",
                        type=str,
                        default=None,
                        help="Path to the compiled Planet executable. Equivalent to PLANET_BIN.")
