...
def finish_query() -> str:
    end_query = "solve_result = MarabouCore.solve(inputQuery, \"\", 0)\n"
    end_query += "if isinstance(solve_result, tuple) and len(solve_result) == 2:\n"
    end_query += "\tvars1, stats1 = solve_result\n"
    end_query += "elif isinstance(solve_result, tuple) and len(solve_result) >= 3:\n"
    end_query += "\tif isinstance(solve_result[0], dict):\n"
    end_query += "\t\tvars1, stats1 = solve_result[0], solve_result[1]\n"
    end_query += "\telse:\n"
    end_query += "\t\tvars1, stats1 = solve_result[1], solve_result[2]\n"
    end_query += "else:\n"
    end_query += "\tvars1, stats1 = solve_result, None\n\n"
...
def _normalize_exit_code(exit_code: Any) -> str | None:
    if exit_code is None:
        return None
    text = str(exit_code).strip()
    if not text:
        return None
    lowered = text.lower()
    if "timeout" in lowered:
        return "timeout"
    if "unsat" in lowered or "infeasible" in lowered:
        return "unsat"
    if lowered == "sat" or "sat" in lowered:
        return "sat"
    return lowered
...
def _looks_like_stats(value: Any) -> bool:
    ...
    for accessor_name in (
        ...
        if callable(getattr(value, accessor_name, None)):
            return True
    for attribute_name in ("exitCode", "ExitCode", "timedOut"):
        if hasattr(value, attribute_name):
            return True
    return False


def _extract_exit_code_from_stats(stats: Any) -> str | None:
    if stats is None:
        return None
    accessor = getattr(stats, "getExitCode", None)
    if callable(accessor):
        try:
            return str(accessor())
        except Exception:
            pass
    for attr_name in ("exitCode", "ExitCode"):
        if hasattr(stats, attr_name):
            try:
                return str(getattr(stats, attr_name))
            except Exception:
                pass
    return None


def _unpack_solve_result(solve_result: Any) -> Tuple[Any, Any, str | None]:
    if not isinstance(solve_result, tuple):
        return solve_result, None, None

    if len(solve_result) == 2:
        return solve_result[0], solve_result[1], None

    if len(solve_result) >= 3:
        first, second, third = solve_result[:3]

        if isinstance(first, dict):
            return first, second, _normalize_exit_code(third)
        if isinstance(second, dict):
            return second, third, _normalize_exit_code(first)

        if _looks_like_stats(second) and not _looks_like_stats(first):
            return first, second, _normalize_exit_code(third)
        if _looks_like_stats(third) and not _looks_like_stats(second):
            return second, third, _normalize_exit_code(first)
...
def _infer_query_result(values: Any, stats: Any, explicit_exit_code: str | None) -> str:
    normalized_exit = _normalize_exit_code(explicit_exit_code)
    if normalized_exit == "sat":
        return "SAT"
    if normalized_exit == "unsat":
        return "UNSAT"
    if normalized_exit == "timeout":
        raise RuntimeError("Marabou solve timed out")

    normalized_stats_exit = _normalize_exit_code(_extract_exit_code_from_stats(stats))
    if normalized_stats_exit == "sat":
        return "SAT"
    if normalized_stats_exit == "unsat":
        return "UNSAT"
    if normalized_stats_exit == "timeout":
        raise RuntimeError("Marabou solve timed out")

    has_timed_out = getattr(stats, "hasTimedOut", None)
    if callable(has_timed_out):
        try:
            if bool(has_timed_out()):
                raise RuntimeError("Marabou solve timed out")
        except RuntimeError:
            raise
        except Exception:
            pass
    if hasattr(stats, "timedOut"):
        try:
            if bool(getattr(stats, "timedOut")):
                raise RuntimeError("Marabou solve timed out")
        except RuntimeError:
            raise
        except Exception:
    ...
    if isinstance(values, dict):
        return "SAT" if len(values) > 0 else "UNSAT"

    raise RuntimeError(
        "Unable to interpret Marabou solve result with values type '{}' and stats type '{}'".format(
            type(values).__name__,
            type(stats).__name__ if stats is not None else "NoneType",
        )
    )
...
def _try_save_query(input_query: Any, save_query_path: str | os.PathLike[str] | None, verbose: bool) -> None:
    ...
    try:
        ...
        MarabouCore.saveQuery(input_query, resolved_path.as_posix())
    except Exception as exc:
        if verbose:
            print(
                "warning: failed to save Marabou query to {}: {}: {}".format(
...
def _collect_ge_equation_type_candidates() -> list:
    candidates = []
    for attr_name in ("GE", "GEQ"):
        if hasattr(MarabouCore.Equation, attr_name):
            candidates.append(getattr(MarabouCore.Equation, attr_name))

    equation_type_cls = getattr(MarabouCore.Equation, "EquationType", None)
    if equation_type_cls is not None:
        for attr_name in ("GE", "GEQ"):
            if hasattr(equation_type_cls, attr_name):
                candidates.append(getattr(equation_type_cls, attr_name))
    return candidates


def _build_ge_threshold_equation(
        variable: int,
        coefficient: float = 1.0,
        threshold: float = 0.0,
):
    equation = None
    for candidate in _collect_ge_equation_type_candidates():
        if isinstance(candidate, type):
            continue
        try:
            equation = MarabouCore.Equation(candidate)
            break
        except Exception:
            continue
    ...
    if equation is None:
        ...
        set_type = getattr(equation, "setType", None)
        type_was_set = False
        if callable(set_type):
            for candidate in _collect_ge_equation_type_candidates():
                ...
                try:
                    ...
                    continue
        if not type_was_set:
            raise RuntimeError("MarabouCore.Equation does not expose a GE/GEQ equation type")

    equation.addAddend(float(coefficient), int(variable))
    equation.setScalar(float(threshold))
    return equation


def _is_acasxu_adversarial_network(net: Sized) -> bool:
    return (
        len(getattr(net.layers[0], "nodes", [])) == 5
        and len(getattr(net.layers[-1], "nodes", [])) == 5
    )
...
def handle_adversarial(net, test_property) -> Tuple:
    if _is_acasxu_adversarial_network(net):
        minimum = 100
        for index, bound in test_property["output"]:
            if bound["Lower"] < minimum:
                minimum = bound["Lower"]
        ...
        minimum_node = net.layers[-1].nodes[minimum_ind]
        new_last_hidden_nodes = []
        non_target_indices = list(set(range(len(net.layers[-1].nodes))).difference([minimum_ind]))
        print("labeled as {}".format(minimum_ind))
        for node in net.layers[-2].nodes:
            node.out_edges = []
        for i, j in enumerate(non_target_indices):
            node_name = f"x_{len(net.layers) - 1}_{i}"
            new_node = ARNode(name=node_name, ar_type="",
                              in_edges=[], out_edges=[])
            another_node = net.layers[-1].nodes[j]
        ...
        test_property["_adversarial_target_label"] = minimum_ind
        test_property["_adversarial_non_target_labels"] = non_target_indices
        test_property["_adversarial_output_order"] = "lower_is_better"
        test_property["_adversarial_query_mode"] = "bounds"
        test_property["_adversarial_margin"] = "raw_sum_relu_nonwinner_minus_winner"
        test_property["_adversarial_violation_operator"] = "ge"
        test_property["_cegar_nn_output_constraint_compatible"] = True
        test_property["_adversarial_winner_value"] = minimum
        net.generate_name2node_map()
        net.weights = net.generate_weights()
        net.biases = net.generate_biases()
        return net, test_property

    explicit_target_label = test_property.get("_adversarial_reference_label")
    if explicit_target_label is not None:
        minimum_ind = int(explicit_target_label)
        minimum = None
    else:
        minimum = 100
        for index, bound in test_property["output"]:
    ...
    minimum_node = net.layers[-1].nodes[minimum_ind]
    new_last_hidden_nodes = []
    non_target_indices = sorted(set(range(len(net.layers[-1].nodes))).difference([minimum_ind]))
    ...
    for i, j in enumerate(non_target_indices):
        ...
        new_node = ARNode(name=node_name, ar_type="",
                          ...
                          bias=net.layers[-1].nodes[j].bias - minimum_node.bias)
        another_node = net.layers[-1].nodes[j]
        minimum_in_edges = sorted(minimum_node.in_edges, key=lambda x: x.src)
        another_in_edges = sorted(another_node.in_edges, key=lambda x: x.src)
        assert (len(minimum_in_edges) == len(another_in_edges))
        ...
        new_last_hidden_nodes.append(new_node)
    net.layers[-1].type_name = "output"
    net.layers[-1].nodes = new_last_hidden_nodes
    # print(net.layers[-1])
    # print("###############################")
    # print(net.layers[-2])
    test_property["output"] = [
        (i, {"Lower": threshold}) for i in range(len(new_last_hidden_nodes))
    ]
    test_property["_adversarial_target_label"] = minimum_ind
    test_property["_adversarial_non_target_labels"] = non_target_indices
    test_property["_adversarial_output_order"] = test_property.get(
        "_adversarial_output_order",
        "higher_is_better",
    )
    test_property["_adversarial_query_mode"] = "disjunction"
    test_property["_adversarial_margin"] = "logit_non_target_minus_reference"
    test_property["_adversarial_violation_operator"] = "ge"
    test_property["_adversarial_threshold"] = threshold
    # print(test_property["output"])
    net.generate_name2node_map()
    net.weights = net.generate_weights()
    net.biases = net.generate_biases()
...
# Followings are the CEGAR_NN5 method for ACAS Xu
...
def get_query(
        ...
        marabou_timeout_seconds: int = DEFAULT_MARABOU_TIMEOUT_SECONDS,
    ...
    """
    if MarabouCore is None or createOptions is None:
        raise ModuleNotFoundError(
            "Failed to import 'maraboupy'. Set NARV_MARABOU_DIR/MARABOU_DIR "
            "or pass --marabou-dir before running Marabou verification."
        )
    property_type = test_property["type"]
    ...
    for key, value in test_property.items():
        if key == "type" or str(key).startswith("_"):
            continue
        else:
    ...
    if (
        ...
        for var_index, bounds in test_property["output"]:
            output_var = out_layer_var_index + var_index
            if violation_operator == "le":
                upper_threshold = float(bounds.get("Upper", 0.0))
                disjunction_clauses.append([
                    _build_ge_threshold_equation(
                        output_var,
                        coefficient=-1.0,
                ...
                if verbose:
                    print(
                        "add adversarial disjunct: output var {} <= {}".format(
                            output_var,
                            upper_threshold,
                        )
                    )
            elif violation_operator == "ge":
                lower_threshold = float(bounds.get("Lower", 0.0))
                disjunction_clauses.append([
                    _build_ge_threshold_equation(
                        output_var,
                        coefficient=1.0,
        ...
        if not disjunction_clauses:
            raise Exception("adversarial property must include at least one output disjunct")
        MarabouCore.addDisjunctionConstraint(inputQuery, disjunction_clauses)
    ...
    options = createOptions(
        verbosity=0,
        timeoutInSeconds=int(marabou_timeout_seconds),
    )
    _try_save_query(inputQuery, save_query_path=save_query_path, verbose=verbose)
    solve_result = MarabouCore.solve(inputQuery, options)
    vars1, stats1, explicit_exit_code = _unpack_solve_result(solve_result)
    result = _infer_query_result(
        values=vars1,
        stats=stats1,
...
def get_bounds(self, nodes2variables, test_property) -> str:
    ...
    for layer_name, bounds_list in test_property.items():
        if layer_name == "type" or str(layer_name).startswith("_"):
            continue
        for (var_index, var_bounds_dict) in bounds_list:
            lower_bound = var_bounds_dict.get("Lower", "-large")
            upper_bound = var_bounds_dict.get("Upper", "large")
